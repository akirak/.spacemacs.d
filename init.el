...
values."
  ...
  (setq-default
   ...
   ;; over any automatically added closing parenthesis, bracket, quote, etcéÂ¥¦
...
you should place your code here."
  ...
  ;; Add my directory for snippets
  (let ((my-yas-snippet-dir (expand-file-name "~/github/yasnippet-snippets")))
    (when (file-exists-p my-yas-snippet-dir)
      (add-to-list 'yas-snippet-dirs my-yas-snippet-dir)))

  (lexical-let*
      ((inbox-org-file "~/Dropbox/org/todo/Inbox.org")
       (personal-journal-repository "~/work/keybase/akirak/personal-journal")
       (masterplan-directory (f-expand "masterplan" personal-journal-repository))
       (home-plan (f-expand "SimplifyLife.org" masterplan-directory))
       )

    (spacemacs|define-custom-layout "@masterplan"
      :binding "m"
      :body
      (defun akirak/add-to-masterplan-persp ()
        ;; Check if the buffer is a file in the directory
        (when (f-child-of? buffer-file-name masterplan-directory)
          (persp-add-buffer (current-buffer))))
      (add-hook 'org-mode-hook #'akirak/add-to-masterplan-persp)
      (cl-loop for buf in (buffer-list)
               do (with-current-buffer buf
                    (when (eq major-mode 'org-mode) (akirak/add-to-masterplan-persp))))
      (call-interactively
       (neotree-dir masterplan-directory))
      )
